...
@section('content')
    ...
    <div class="col-12 no-print">
        <h3 class="mb-2 page-title">WCF Sheet — {{ $payroll->period }}</h3>
    ...
    <div id="form1" class="wrapper wrapper-content animated fadeInRight">
        <div class="ibox-content payroll-print-area">
            ...
            <div class="text-center mb-3">
                <h3 style="font-weight:bold;">WCF SHEET</h3>
            ...
            <div class="table-responsive">
                <table class="table table-bordered payroll-sheet-table">
                    ...
                    <tbody>
                        @php $grandGross=$grandWcf=0; @endphp
                        @foreach ($companiesGrouped as $companyName => $companyLines)
                            <tr class="company-row">
                                <td colspan="5">COMPANY SITE: {{ strtoupper($companyName) }}</td>
                            </tr>
                            @php
                            ...
                            @foreach ($workPointsGrouped as $workPointName => $workPointLines)
                                <tr class="workpoint-row">
                                    <td colspan="5">WORK POINT: {{ strtoupper($workPointName) }}</td>
                                </tr>
                                @php $subGross=$subWcf=0; @endphp
                                @foreach ($workPointLines as $k => $l)
                                    @php
                                        $subGross += (float) ($l->gross ?? 0);
                                        $subWcf += (float) ($l->wcf ?? 0);
                                    @endphp
                                    <tr>
                                        <td>{{ $k + 1 }}</td>
                                        <td>{{ optional($l->user)->name ?? '-' }}</td>
                                        <td>{{ optional($l->user)->wcfNo ?? (optional($l->user)->wcf_no ?? '-') }}</td>
                                        <td class="text-right">{{ number_format($l->gross ?? 0, 2) }}</td>
                                        <td class="text-right">{{ number_format($l->wcf ?? 0, 2) }}</td>
                                    </tr>
                                @endforeach
                                @php
                                    $grandGross += $subGross;
                                ...
                                <tr class="subtotal-row">
                                    ...
                                    <td class="text-right">{{ number_format($subWcf, 2) }}</td>
                                </tr>
                            @endforeach
                        @endforeach
