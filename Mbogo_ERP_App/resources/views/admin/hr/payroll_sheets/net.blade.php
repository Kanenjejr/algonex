...
@section('content')
    ...
    <div class="col-12 no-print">
        <h3 class="mb-2 page-title">Payroll Net Paysheet — {{ $payroll->period }}</h3>
    ...
    <div id="form1" class="wrapper wrapper-content animated fadeInRight">
        <div class="ibox-content payroll-print-area">
            ...
            <div class="text-center mb-3">
                <h3 style="font-weight:bold;">PAYROLL NET PAYSHEET</h3>
            ...
            <div class="table-responsive">
                <table class="table table-bordered payroll-sheet-table">
                    ...
                    <tbody>
                        @php $grandGross=$grandDeduction=$grandNet=0; @endphp
                        @foreach ($companiesGrouped as $companyName => $companyLines)
                            ...
                            @foreach ($workPointsGrouped as $workPointName => $workPointLines)
                                ...
                                @php $subGross=$subDeduction=$subNet=0; @endphp
                                @foreach ($workPointLines as $k => $l)
                                    @php
                                        $totalDeduction = (float) ($l->total_deductions ?? 0);
                                        if ($totalDeduction <= 0) {
                                            $totalDeduction =
                                                (float) ($l->paye ?? 0) +
                                                (float) ($l->nssf_employee ?? 0) +
                                                (float) ($l->loan_deduction ?? 0) +
                                                (float) ($l->heslb_deduction ?? 0);
                                        }
                                        $subGross += (float) ($l->gross ?? 0);
                                        $subDeduction += $totalDeduction;
                                        $subNet += (float) ($l->net_pay ?? 0);
                                    @endphp
                                    <tr>
                                        <td>{{ $k + 1 }}</td>
                                        <td>{{ optional($l->user)->name ?? '-' }}</td>
                                        <td>{{ optional($l->user)->accName ?? '—' }}</td>
                                        <td>{{ optional($l->user)->accNo ?? '—' }}</td>
                                        <td>{{ optional($l->user)->bankName ?? (optional($l->user)->bank_name ?? '—') }}
                                        </td>
                                        <td class="text-right">{{ number_format($l->gross ?? 0, 2) }}</td>
                                        <td class="text-right">{{ number_format($totalDeduction, 2) }}</td>
                                        <td class="text-right">{{ number_format($l->net_pay ?? 0, 2) }}</td>
                                    </tr>
                                @endforeach
                                @php
                                    $grandGross += $subGross;
                                    $grandDeduction += $subDeduction;
                                    $grandNet += $subNet;
                                @endphp
                                <tr class="subtotal-row">
                                    <td colspan="5" class="text-right">TOTAL FOR {{ strtoupper($workPointName) }}</td>
                                    <td class="text-right">{{ number_format($subGross, 2) }}</td>
                                    <td class="text-right">{{ number_format($subDeduction, 2) }}</td>
                                    <td class="text-right">{{ number_format($subNet, 2) }}</td>
                                </tr>
                            @endforeach
                        @endforeach
                        <tr class="grand-total-row">
                            <td colspan="5" class="text-right">GENERAL TOTAL</td>
                            <td class="text-right">{{ number_format($grandGross, 2) }}</td>
                            <td class="text-right">{{ number_format($grandDeduction, 2) }}</td>
