...
@section('content')
    ...
    <div class="col-12 no-print">
        <h3 class="mb-2 page-title">Loans / Advances Deduction Sheet — {{ $payroll->period }}</h3>
    ...
    <div id="form1" class="wrapper wrapper-content animated fadeInRight">
        <div class="ibox-content payroll-print-area">
            ...
            <div class="text-center mb-3">
                <h3 style="font-weight:bold;">LOANS / ADVANCES DEDUCTION SHEET</h3>
                ...
                <table class="table table-bordered payroll-sheet-table">
                    <thead>
                        <tr>
                            ...
                            <th>Staff</th>
                            <th>Work Point</th>
                            <th class="text-right">Loan / Advance Monthly Deduction</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $grandLoans=0; @endphp
                        @foreach ($companiesGrouped as $companyName => $companyLines)
                            <tr class="company-row">
                                <td colspan="4">COMPANY SITE: {{ strtoupper($companyName) }}</td>
                            </tr>
                            @php
                            ...
                            @foreach ($workPointsGrouped as $workPointName => $workPointLines)
                                <tr class="workpoint-row">
                                    <td colspan="4">WORK POINT: {{ strtoupper($workPointName) }}</td>
                                </tr>
                                @php $subLoans=0; @endphp
                                @foreach ($workPointLines as $k => $l)
                                    @php $subLoans+=(float)($l->loan_deduction ?? 0); @endphp
                                    <tr>
                                        <td>{{ $k + 1 }}</td>
                                        <td>{{ optional($l->user)->name ?? '-' }}</td>
                                        <td>{{ optional(optional($l->user)->workpoint)->work_name ?? '-' }}</td>
                                        <td class="text-right">{{ number_format($l->loan_deduction ?? 0, 2) }}</td>
                                    </tr>
                                @endforeach
                                @php $grandLoans+=$subLoans; @endphp
                                <tr class="subtotal-row">
                                    <td colspan="3" class="text-right">TOTAL FOR {{ strtoupper($workPointName) }}</td>
                                    <td class="text-right">{{ number_format($subLoans, 2) }}</td>
                                </tr>
                            @endforeach
                        @endforeach
                        <tr class="grand-total-row">
                            <td colspan="3" class="text-right">GENERAL TOTAL</td>
                            <td class="text-right">{{ number_format($grandLoans, 2) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
